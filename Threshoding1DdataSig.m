function [sig, optimal_parameters_table] = Threshoding1DdataSig(original_signal, sigma_values, kernel_width_values, a, b, num_trials, v,ww )
    ...
    for sigma = sigma_values
        for kernel_width = kernel_width_values
            ...
            for trial = 1:num_trials
                % Add Gaussian noise
                noisy_signal = original_signal + sigma * randn(size(original_signal));
               ...
               % v_a = p_a .* (1 - p_a) ./ (numel(original_signal) * f(F_inv(p_a)).^2);
                ...
                recovery_sub = a - max(-v, min(v, F_inv(p_a))); %min(v,max(-v,norminv(1- pb(i))))
            ...
            %va(find(~va)) = .005; vb(find(~vb)) = 0.005;
