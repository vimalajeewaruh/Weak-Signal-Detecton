...
function [sig, optimal_parameters_table] = Threshoding2DTestSig(original_image, sigma_values, kernel_width_values, a, b, num_trials, v)
    % Image size
    ...
    for sigma = sigma_values
        for kernel_width = kernel_width_values
            ...
            for trial = 1:num_trials
                ...
                noisy_image = original_image + sigma * randn(size(original_image));
    
                % Thresholding
                sub_threshold = double(noisy_image < a);
                sup_threshold = double(noisy_image > b);
            ...
            va = var_sub_avg / num_trials; %va(find(~va)) = 1e-5;
            vb = var_sup_avg / num_trials; %vb(find(~vb)) = 1e-5;
            ...
            % Compute MSE
            mse_sub = mean((original_image(:) - recovery_sub_avg(:)).^2);
            mse_sup = mean((original_image(:) - recovery_sup_avg(:)).^2);
            mse_joint = mean((original_image(:) - joint_recovery(:)).^2);
